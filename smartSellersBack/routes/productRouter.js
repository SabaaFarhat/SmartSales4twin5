...
const transporter = nodemailer.createTransport(
  sendgridTransport({
    auth: {
      api_key:
        "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",
    },
  })
);
// Multer config ************************************************
...
Router.post(
  ...
  async (req, res) => {
    ...
    const newProduct = new Product({
      ...
      // image: url  + '/routes/images/' + req.file.filename,
      //req.body
    });
...
//GET ALL PRODUCTS
Router.get("/afficheProducts", async (req, res) => {
...
//reponse
Router.post("/addreponse/:idProd/:id", async (req, res) => {
  ...
  newReponse
    .save()
    .then(() => res.json("reponse added!"))
    .catch((err) => res.status(400).json("Error: " + err));
  Comment.findById(commentId)
    .then((commentt) => {
      commentt.reponses.push(newReponse);
      commentt
        .save()
        .then(() => res.json("reponse added to comment"))
        .catch((err) => res.status(400).json("Error: " + err));
    })
...
//UPDATE
Router.put("/update/:id", checkUser, async (req, res) => {
  try {
...
//GET COMMENTAIRE BY IDCommentaire
Router.get("/findco/:id", (req, res) => {
...
//GET COMMENTAIRE BY PRODUCT
Router.get("/findcomm/:id", (req, res) => {
  const comment = Comment.find({product: req.params.id}, (err, doc) => {
    if (err) {
      console.log(err);
    } else {
      res.send(doc);
...
Router.get("/afficheComments",  async (req, res) => {
  // if (req.query.page && req.query.limit) {
